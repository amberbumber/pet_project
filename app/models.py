...
# т.к. это вспомогательная таблица (не содержит новой информации) - создается без класса модели
# таблица для связи подписчик-на кого подписан
...
class User(UserMixin, db.Model):
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue('app.tasks.' + name, self.id, *args, **kwargs)
...
class Task(db.Model):
    ...
    def get_rq_job(self):
        ...
        except (redis.exceptions.RedisError, rq_exceptions.NoSuchJobError):
            return None
        return rq_job
